...
func doltgresProllyMapIterator(ctx *sql.Context, keyDesc val.TupleDesc, ns tree.NodeStore, root tree.Node, rang DoltgresRange) (prolly.MapIter, error) {
	searchRow := sql.NewSqlRow(len(keyDesc.Types))
...
func doltgresMapSearchKeyToRow(ctx context.Context, key val.Tuple, keyDesc val.TupleDesc, ns tree.NodeStore, row sql.LazyRow) (err error) {
	var val interface{}
	for i := 0; i < row.Count(); i++ {
		val, err = tree.GetField(ctx, keyDesc, i, key, ns)
		if err != nil {
			return err
		}
		row.SetSqlValue(0, val)
	}
	return
}
